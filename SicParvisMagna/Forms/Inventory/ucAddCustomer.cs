...
namespace SicParvisMagna.Forms
{
    public partial class ucAddCustomer : UserControl
    {
        ...
        private void searchContact()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(cbxCustomer.Text))
                {
                    if (!whereAdded)
                    {
                        query += "   WHERE User_id like '%" + cbxCustomer.Text + "%'";
                        whereAdded = true;
                    }
                    else
                        query += "   AND User_id like '%" + cbxCustomer.Text + "%'";
                }
                SqlDataAdapter da = new SqlDataAdapter();
                DataTable dt = new DataTable();
        ...
        {
            CustomerDAL objDAL = new CustomerDAL();
            cbxCustomer.DataSource = objDAL.LoadAll();
            cbxCustomer.ValueMember = "User_id";
            cbxCustomer.DisplayMember = "First_name";
        ...
        private void ucAddCustomer_Load(object sender, EventArgs e)
        {
          ...
          //  loadCbxCities();
            ...
            if (UserID == null)
            {
                EnableTab(mtcCustomer.TabPages["Customer"], false);
                EnableTab(mtcCustomer.TabPages["Contact"], false);
                
            }

            if (mtcCustomer.SelectedTab == mtcCustomer.TabPages["Customer"])
            {

                loadUsers();
            }

            if (mtcCustomer.SelectedTab == mtcCustomer.TabPages["Contact"])
            {
                loadContacts();
            }
        }
        ...
        private void gridContact_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rowindex >= 0)
            {
                ...
                cbxCustomer.SelectedValue = UserID;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (mtcCustomer.SelectedTab == mtcCustomer.TabPages["Contacts"])
            {
                saveContact();
            }
        }
