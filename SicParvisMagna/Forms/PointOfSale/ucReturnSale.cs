...
{
    public partial class ucReturnSale : UserControl
    {
        bool validated = true;
       ...
       // GridViewComboBoxColumn Quantity = new GridViewComboBoxColumn();
...

        {
            ...
            debug = true;
            Sale_Order_Return_DetailDAL obj = new Sale_Order_Return_DetailDAL();
            ...
            for (int x = 0; x < dt.Rows.Count; x++)
            {
                ...
                gridSales.Rows[x].Cells["SaleAmount"].Value = saleAmount;
                //
        ...
        public void loadForm_byID(Guid id)
        {
           ...
           Guid.TryParse(ID.ToString(), out return_order_id);
               ...
               Guid.TryParse(OrgID.ToString(), out organization_id);
                ...
                loadCustomer();
                var partyID = dt.Rows[x]["customer_id"];
               Guid.TryParse(partyID.ToString(), out party_id);
                cbx_Customer.SelectedValue = party_id;
        ...
        {
            //Manage Permissions
            pgURL = "Manage Sales Return Orders"; 
            //for Account Type
           ...
           // clearAll();
             Sale_Order_ReturnDAL orderDAL = new Sale_Order_ReturnDAL();
            orderDAL.LoadAll();
            cbx_Organization.SelectedValue = organization_id;
            LoadCmbOrgBranch();
            cbx_branch.SelectedValue = branch_id;

            Sale_Order_Return_DetailDAL detailDAL = new Sale_Order_Return_DetailDAL();
            detailDAL.LoadAll();
            ...
            try
            {
                ...
                Guid.TryParse(cbx_Customer.SelectedValue.ToString(), out party_id);
                cbx_Status.Items.Add("Paid and returned");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateForm())
                {
                    Sale_Order_ReturnBAL objBAL = new Sale_Order_ReturnBAL();
                    GenerateID_Invoice();
                    GenerateID_Order();
                    ...
                    try
                    {
                        Sale_Order_ReturnDAL objDAL = new Sale_Order_ReturnDAL();
                    ...
                    //clearAll();
                    //    
                    Sale_Order_Return_DetailDAL objt = new Sale_Order_Return_DetailDAL();
                    ...
                    for (int x = 0; x < gridSales.RowCount; x++)
                    {
                        ...
                        Sale_Order_Return_DetailsBAL objDetailBAL = new Sale_Order_Return_DetailsBAL();
                        objDetailBAL.sale_order_Return_id = return_order_id;
                        ...
                        objDetailBAL.purchase_amount = Convert.ToDouble(gridSales.Rows[x].Cells["UnitPrice"].Value.ToString());
                        ...
                        objDetailBAL.tax_amount = Convert.ToDouble(gridSales.Rows[x].Cells["PriceWithTax"].Value.ToString());
                        ...
                        Sale_Order_Return_DetailDAL objDetailDAL = new Sale_Order_Return_DetailDAL();
                        ...
                        obj.quantity = Convert.ToDouble(gridSales.Rows[x].Cells["Quantity"].Value.ToString());
                        ...
                        obj.expiryDate = Convert.ToDateTime("02-02-2020");
                        obj.Addby = Program.User_id;
                        obj.AddDate = DateTime.Now;
                        obj.status = "Return Stock from Sale";
        ...
        private void gridPurchases_CellEndEdit(object sender, GridViewCellEventArgs e)
        {
            ...
            //        gridSales.CurrentColumn = cmb_Product;
        ...
        private void GenerateID_Invoice()
        {
            if (txt_Invoice.Text.Length==0)
            {
                SqlDataReader dr;
                con.Open();
                cmd = new SqlCommand("Select Max(Invoice_no) from Sale_Order_Return  Where  Organization_id = '" + cbx_Organization.SelectedValue+"'" +
        ...
        private void GenerateID_Order()
        {
            if (txt_Order.Text.Length == 0)
            {
                SqlDataReader dr;
                con.Open();
                cmd = new SqlCommand("Select Max(Order_no) from Sale_Order_Return Where  Organization_id = '" + cbx_Organization.SelectedValue + "'" +
                    "  and branch_id  = '" + cbx_branch.SelectedValue + "'", con);
                dr = cmd.ExecuteReader();
                string newId = "";
                if (dr.HasRows)
        ...
        private void gridPurchases_CellEditorInitialized(object sender, GridViewCellEventArgs e)
        {
            ...
            if (e.Column.HeaderText == "Quantity" || e.Column.HeaderText == "Discount Percnetage")
            {
                ...
                try
                {
                    ...
                    //pricing
                    Guid PriceID = Guid.Empty; Guid PID = Guid.Empty;
